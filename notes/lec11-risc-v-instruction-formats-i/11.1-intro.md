...
从本节开始我们要学习RISC-V machine language，这是我们表示汇编指令的二进制格式的方式，以便处理器能够理解。我们学习这个并不是因为我们特别喜欢盯着指令的二进制表示。而是因为当我们尝试构建能够执行这些指令的 RISC-V 处理器时，这将会非常有帮助。我们需要能够用处理器能理解的语言来交流，因为我们没有另一种设计方法。我们仍然没有一个系统，可以这样做计算机，要接受 RISC-V 规范，构建符合规范的处理器。
