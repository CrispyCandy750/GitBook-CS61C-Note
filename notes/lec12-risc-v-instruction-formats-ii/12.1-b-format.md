---
description: 本节内容：1.branch指令需要的参数；2.跳转指令的注意事项；3.PC相对寻址；4.RISC-V的压缩指令；5.B-Format例子；6.为什么B-Format要分开排放imm；7. 所有的B-Format指令编码； 
---
...
1. 这些循环在被翻译成汇编代码时，相对来说长度较短。循环内部的指令==**数量通常少于50条**==，最多可能几百条。
...
- 使用byte吗？不，因为我们==**不想在代码中的某个地方分支到一条指令的中间**==。RISC-V32指令集中的代码由始终为32位宽的指令组成。因此，如果我们分支到与指令开始不对齐的地址，是会出错的。所以我们不想分支到指令的中间。
...
![image-20240607195518726](.image/image-20240607195518726.png)
...
![image-20240607202954853](.image/image-20240607202954853.png)
...
![image-20240607203033238](.image/image-20240607203033238.png)
...
这实际上更容易，因为==**我们通过保持立即数大部分在相同位置来帮助处理器找到立即数**==。因此，我们的寄存器会在相同的位置，并且大部分的立即数也会在相同的位置，这会带来好处。
...
![image-20240607203118124](.image/image-20240607203118124.png)
