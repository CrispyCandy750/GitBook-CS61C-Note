...
![image-20240623201854856](.image/image-20240623201854856.png)
...
![image-20240623202337454](.image/image-20240623202337454.png)

> 同理，在解析Address的时候，首先解析Index

2. 然后查看Tag，发现no valid data

![image-20240623202400680](.image/image-20240623202400680.png)

3. 然后从内存中加载数据到Cache中，并且set tag和valid bit（Tag就使用Address中的Tag赋值）

![image-20240623202450591](.image/image-20240623202450591.png)
...
![image-20240623202606856](.image/image-20240623202606856.png)
...
![image-20240623203257503](.image/image-20240623203257503.png)

Tag = 0, Index = 1, Offset = 12(只看钱两列为第3个word)

1. Index定位Cache的row

![image-20240623203307772](.image/image-20240623203307772.png)

2. 查看valid bit和比较Tag，发现匹配

![image-20240623203344543](.image/image-20240623203344543.png)

3. 根据Offset选取对应的word

![image-20240623203411317](.image/image-20240623203411317.png)

### 3. Read 0x00000034 = 0…00 0..011 0100

初始状态

![image-20240623203521173](.image/image-20240623203521173.png)

2. Index定位Cache的row

![image-20240623203530296](.image/image-20240623203530296.png)

2. valid bit为0，从内存中加载数据到Cache，并设置valid bit和Tag

![image-20240623203559799](.image/image-20240623203559799.png)
...
![image-20240623203742236](.image/image-20240623203742236.png)

1. Index定位Cache的row

![image-20240623203750781](.image/image-20240623203750781.png)

2. Valid bit为1，有效；但是Tag并不匹配

![image-20240623203811392](.image/image-20240623203811392.png)

3. 从内存中加载数据到Cache中，并重新设置Tag

![image-20240623203831184](.image/image-20240623203831184.png)

4. 根据Offset选择对应的word

![image-20240623203849233](.image/image-20240623203849233.png)
