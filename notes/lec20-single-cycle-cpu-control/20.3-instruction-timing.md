...
![image-20240617101759148](.image/image-20240617101759148.png)

- 在时钟的上升沿，更新PC。新值需要经过写入时间(**Clock to output delay**)才能到达输出端
- 在Clock to output delay后，输出端得到了一个有效的PC地址，指向IMEM。

![image-20240617101911640](.image/image-20240617101911640.png)
...
![image-20240617102146860](.image/image-20240617102146860.png)

- Control Logic接受Inst之后读取关键位生成逻辑信号

![image-20240617135129849](.image/image-20240617135129849.png)
...
![image-20240617140643892](.image/image-20240617140643892.png)

在这个时候，同时进行Reg的读操作

![image-20240617140655207](.image/image-20240617140655207.png)

- `Reg[rs1]` 和 `Reg[rs2]`都被送到了多路复用器
- 由于此时已经生成了ASel和BSel，所以可以直接通过

![image-20240617140736071](.image/image-20240617140736071.png)

#### Execute

ALU的两个操作数已经准备就绪，ALUSel也已就绪，执行加法操作

![image-20240617140804446](.image/image-20240617140804446.png)

结果送到Write Back的多路复用器的输入，由于WBSel已经设置好，所以可以直接通过

![image-20240617140834802](.image/image-20240617140834802.png)

#### Write Register

ALU的结果然后被送回到Reg[] 的DataD中

![image-20240617140858810](.image/image-20240617140858810.png)

> 现在，状态更新需要的所有数据和信号都准备就绪，在下一个时钟信号时设置值

![image-20240617140954160](.image/image-20240617140954160.png)
...
![image-20240615100622080](../lec18-single-cycle-cpu-datapath-i/.image/image-20240615100622080.png)

#### Instruction Fetch

> 在PC的输出处， 有一组32根导线。这里不会像逐个显示这32根导线的01序列，而是使用之前介绍过的简写符号。如下图，使用了bundle，用1000,1004来表示01序列的状况

![image-20240615100713564](../lec18-single-cycle-cpu-datapath-i/.image/image-20240615100713564.png)
...
![image-20240615100754872](../lec18-single-cycle-cpu-datapath-i/.image/image-20240615100754872.png)
...
![image-20240615100953760](../lec18-single-cycle-cpu-datapath-i/.image/image-20240615100953760.png)
...
![image-20240617150018884](.image/image-20240617150018884.png)

- instruction Fetch之后送到了Control Logic进行解析，一段时间后设置Control bit

同时进行的操作是读取寄存器

![image-20240617150230249](.image/image-20240617150230249.png)
...
![image-20240617150343329](.image/image-20240617150343329.png)

- 在读取了`Reg[rs1]`和`Reg[rs2]`之后经过一段的时间，执行加法计算

#### Write Register

![image-20240617150421769](.image/image-20240617150421769.png)

- 加法计算结束以后，结果经过wb多路复用器，到达Reg[]的DataD输入端

最后经过一个时钟上升沿，PC和rd更新

![image-20240617150542870](.image/image-20240617150542870.png)
...
![image-20240617151307110](.image/image-20240617151307110.png)

Critical path = t~clk-q~ +max {t~Add~+t~mux~, t~IMEM~+t~Reg~+t~mux~+t~ALU~+t~mux~}+t~setup~ = t~clk-q~ + t~IMEM~+t~Reg~+t~mux~+t~ALU~+t~mux~+t~setup~

下面解释一下各段时间

| time                                | desc                                                         |
| ----------------------------------- | ------------------------------------------------------------ |
| t~clk-q~                            | 时钟上升沿到PC更新完成的延迟                                 |
| t~Add~+t~mux~                       | 执行PC+4并且通过mux的时间                                    |
| t~IMEM~+t~Reg~+t~mux~+t~ALU~+t~mux~ | 从IMEM读取指令，从Reg[]读取数据，Reg[rs1]和Reg[rs2]通过mux，ALU执行加法操作，结果通过WB mux(这里假设Control Logic在读取寄存器之前，所以没有列出) |
| t~setup~                            | Reg[rd]的更新时间                                            |

在这个路径中，显然有t~Add~+t~mux~ < t~IMEM~+t~Reg~+t~mux~+t~ALU~+t~mux~，所以关键路径为 Critical path = t~clk-q~ + t~IMEM~+t~Reg~+t~mux~+t~ALU~+t~mux~+t~setup~
...
![image-20240617152224777](.image/image-20240617152224777.png)

Critical path = t~clk-q~ +max {t~Add~+t~mux~, t~IMEM~+t~Imm~+t~mux~+t~ALU~+t~DMEM~+t~mux~, t~IMEM~+t~Reg~+t~mux~+t~ALU~+t~DMEM~+t~mux~}+t~setup~

下面是三个并行的路径

| time                                        | desc              |
| ------------------------------------------- | ----------------- |
| t~Add~+t~mux~                               | (见add操作的time) |
| t~IMEM~+t~Imm~+t~mux~+t~ALU~+t~DMEM~+t~mux~ | t~Imm~            |
| t~IMEM~+t~Reg~+t~mux~+t~ALU~+t~DMEM~+t~mux~ | t~Reg~            |

后面两个时间关键的就是t~imm~和t~Reg~，哪个时间久，哪条路就是关键路径。
...
![image-20240616203832627](.image/image-20240616203832627.png)

> 改图重点显示了每一个阶段的时间，例如t~IF~, t~ID~等等
...
- Most blocks idle most of the time  f~max,ALU~ = 1/200ps = 5 GHz
