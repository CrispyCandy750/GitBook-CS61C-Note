...
- 该值需要一点时间才能出现在PC的输出端，这里的**写入时间**就是我们所描述的时钟到输出延迟(**Clock to output delay**)。
...
![image-20240617102235420](.image/image-20240617102235420.png)
...
![image-20240617103715261](.image/image-20240617103715261.png)
...
![image-20240617103816428](.image/image-20240617103816428.png)
...
![image-20240617103930042](.image/image-20240617103930042.png)
...
![image-20240607195518726](../lec12-risc-v-instruction-formats-ii/.image/image-20240607195518726.png)
...
![image-20240617111204647](.image/image-20240617111204647.png)

- 在时钟的上升沿，向PC写入一个新值。
- 该值需要一点时间才能出现在PC的输出端，这里的**写入时间**就是我们所描述的时钟到输出延迟(**Clock to output delay**)。
- 在Clock to output delay后，我们在这里得到了一个有效的PC地址，指向IMEM。

![image-20240617125622424](.image/image-20240617125622424.png)
...
![image-20240617130111441](.image/image-20240617130111441.png)

得到的信号如下

![image-20240617130120021](.image/image-20240617130120021.png)
...
![image-20240617130659624](.image/image-20240617130659624.png)
...
![image-20240617130558049](.image/image-20240617130558049.png)

- Branch Comp比较`Reg[rs1]`和`Reg[rs2]`的内容并将结果BrEq输入到Control Logic

![image-20240617130815224](.image/image-20240617130815224.png)

- Control Logic根据BrEq生成了PCSel

![image-20240617130845073](.image/image-20240617130845073.png)

- ALU执行完PC + Immediate并将结果送回到多路复用器

> 此时准备好了状态更新前所有的数据：PC + 4，PC + Immediate，PCSel。只待更新了

![image-20240617131006110](.image/image-20240617131006110.png)
